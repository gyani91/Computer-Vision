...
%
    ...
    predict_particles = priorbelief_particles + uk + simpar.odometry_stdev*randn(size(priorbelief_particles));
...
%
    ...
    random_number = rand(1)/simpar.numberOfParticles;
    sum = particle_weights(1);
    index = 1;
    for i = 1:simpar.numberOfParticles
        while (random_number + (i-1)/simpar.numberOfParticles) > sum
            index = index + 1;
            sum = sum + particle_weights(index);
        end
