...
figure(1)
imshow(img); hold on;
scatter(projected_pts(1,:), projected_pts(2,:), 20, 'r', 'filled');
hold off;
...
% Create Video with the Image Sequence

outputVideo = VideoWriter(fullfile('../','ARCube.avi'));
outputVideo.FrameRate = 30;
open(outputVideo)

for img_index = 1:length(imageNames)
    img = rgb2gray(imread(fullfile(workingDir,'images',imageNames{img_index})));
    %img = rgb2gray(imread(['../data/images/',sprintf('img_%04d.jpg',img_index)]));
    ...
    offset_x = square_size * 3; offset_y = square_size;
    s = 2 * square_size;
    ...
    figure(3)
    imshow(img_undistorted_vectorized); hold on;
