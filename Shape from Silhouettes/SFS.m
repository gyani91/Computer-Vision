...
% define bounding box
bbox = [.3 -.15 -2; 2.2 1.2 2.5]; % [minX minY minZ; maxX maxY maxZ];
volumeX = 64;
volumeY = 64;
volumeZ = 128;
...
if computeVisualHull
    ...
    for x=1:volumeX
        for y=1:volumeY
            for z=1:volumeZ
                voxels = [x-0.5 y-0.5 z-0.5 1]';
                %add one to volume if projection is within silhouette region
                for n=1:numCameras                            
                    projVoxels = Ps{n}*T*voxels;
                    projVoxels = round(projVoxels ./ projVoxels(3));
                    if (projVoxels(2) <= size(sils{n},1)) && (projVoxels(2) > 0) && (projVoxels(1) <= size(sils{n},2)) && (projVoxels(1) > 0)
                        volume(x,y,z) = volume(x,y,z) + sils{n}(projVoxels(2),projVoxels(1));               
                    end
                end
            end
        end
